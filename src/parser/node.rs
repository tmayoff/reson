...
impl Node {
    ...
    pub fn id_node(tok: Token) -> Self {
        if let Token::ID(id) = tok.clone() {
            return Node::new(tok, NodeType::IDNode { value: id });
        }
        unreachable!()
    }
    ...
    pub fn number_node(tok: Token) -> Self {
        if let Token::Number(value) = tok {
            return Node::new(tok, NodeType::NumberNode { value });
        }
        unreachable!()
    }

    pub fn string_node(tok: Token) -> Self {
        if let Token::String(value) = tok.clone() {
            return Node::new(tok, NodeType::StringNode { value });
        }
        unreachable!();
    }
